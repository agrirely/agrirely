// src/data/contactContent.js

export const contactContent = {
    hero: {
      heading: "Contact Us",
      subheading: "How can we help?",
      description:
        "We are here to assist you with any questions about our products and services, provide support, or share more about how we are making a meaningful difference. We would be delighted to hear from you.",
    },
  
    reachUs: {
      heading: "Reach Us",
      global: "US, UAE, Mauritius, Singapore, Hong Kong",
      india: "Delhi NCR, Mumbai",
      corporateHQ: "Pune, India",
      email: "info@agrirely.com",
    },
  
    otherWaysToConnect: {
      heading: "Other Ways to Connect",
      tabs: [
        {
          id: "msg",
          label: "Send us a Message",
          fields: ["Name", "Email", "Message"],
        },
        {
          id: "careers",
          label: "Careers",
          note: "Job listings / upload resume form",
        },
        {
          id: "quote",
          label: "Quote Request",
          note: "Commodity, quantity, destination quote form",
        },
        {
          id: "supplier",
          label: "Supplier Opportunities",
          note: "Supplier onboarding / partnership form",
        },
      ],
    },
  };